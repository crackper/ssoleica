...
var loadConfigFile = function (type) {
	...
	if(type == 'user') {
		var url = './filemanager/scripts/filemanager.config.remote.js';
	} else {
		var url = './filemanager/scripts/filemanager.config.js.default'
	}
...
if($.urlParam('langCode') != 0 && file_exists ('filemanager/scripts/languages/'  + $.urlParam('langCode') + '.js')) config.options.culture = $.urlParam('langCode');

var lg = [];
$.ajax({
  url: 'filemanager/scripts/languages/'  + config.options.culture + '.js',
  async: false,
  dataType: 'json',
...
$(function(){
	...
	if(config.edit.enabled) {
		loadCSS('./filemanager/scripts/CodeMirror/lib/codemirror.css');
		loadCSS('./filemanager/scripts/CodeMirror/theme/' + config.edit.theme + '.css');
		loadJS('./filemanager/scripts/CodeMirror/lib/codemirror.js');
		loadJS('./filemanager/scripts/CodeMirror/addon/selection/active-line.js');
		loadJS('./filemanager/scripts/CodeMirror/dynamic-mode.js');
	}
	...
	/** load searchbox */
	if(config.options.searchBox === true)  {
		loadJS("./filemanager/scripts/filemanager.liveSearch.min.js");
	} else {
		$('#search').remove();
	}
