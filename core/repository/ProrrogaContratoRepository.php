...
class ProrrogaContratoRepository extends Repository{
    ...
    public function getProrrogasContrato($contrato_id = 0)
    {
        $query = ProrrogaContrato::join('trabajador as s','prorroga_contrato.solicita_id','=','s.id')
                                    ->join('trabajador as a','prorroga_contrat.aprueba_id','=','a.id')
                                    ->join('month as m','prorroga_contrato.month_id','=','m.id')
                                    ->join('contrato c','prorroga_contrato.contrato_id','=','c.id')
                                    ->where('prorroga_contrato.id',$contrato_id)
                                    ...
                                    ->addSelect('c.nombre_contrato as contratro');
